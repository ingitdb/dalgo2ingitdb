...
func TestDB_Get_MissingRecordMarksNotFound(t *testing.T) {
	...
	rec := record.NewRecordWithData(record.NewKeyWithID("countries", "atlantis"), map[string]any{})
	// Per the dalgo Getter contract, Get on a missing record returns
	// record.ErrRecordNotFound AND marks the record as not existing.
...
func TestDB_Exists_HitAndMiss(t *testing.T) {
	...
	got, err := db.Exists(ctx, record.NewKeyWithID("countries", "france"))
	if err != nil {
	...
	got, err = db.Exists(ctx, record.NewKeyWithID("countries", "atlantis"))
	if err != nil {
...
func TestDB_GetMulti_LoadsEach(t *testing.T) {
	...
	r1 := record.NewRecordWithData(record.NewKeyWithID("countries", "france"), map[string]any{})
	r2 := record.NewRecordWithData(record.NewKeyWithID("countries", "japan"), map[string]any{})
	r3 := record.NewRecordWithData(record.NewKeyWithID("countries", "atlantis"), map[string]any{})
	if err := db.GetMulti(context.Background(), []record.Record{r1, r2, r3}); err != nil {
...
func TestDB_RunReadwriteTransaction_InsertSetDelete(t *testing.T) {
	...
	delErr := db.RunReadwriteTransaction(ctx, func(_ context.Context, tx dal.ReadwriteTransaction) error {
		return tx.Delete(ctx, record.NewKeyWithID("countries", "france"))
	})
	if delErr != nil {
...
func TestDB_RunReadwriteTransaction_UpdateAndUpdateRecord(t *testing.T) {
	...
	err := db.RunReadwriteTransaction(ctx, func(_ context.Context, tx dal.ReadwriteTransaction) error {
		return tx.Update(ctx, record.NewKeyWithID("countries", "japan"),
			[]update.Update{update.ByFieldName("population", 124000000)})
	})
	if err != nil {
		t.Fatalf("Update: %v", err)
	}
	got := record.NewRecordWithData(record.NewKeyWithID("countries", "japan"), map[string]any{})
	if err := db.Get(ctx, got); err != nil {
